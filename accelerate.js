...
Elm.Accelerate.make = function (_elm) {
   ...
   var _op = {};
   var accelerationToNote = function (acceleration) {    return $Basics.round(200 + $Basics.abs((acceleration.x + acceleration.y + acceleration.z) * 100));};
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "NoOp": return model;
         case "Click": return _U.update(model,{mute: $Basics.not(model.mute)});
         default: var _p1 = _p0._0;
           return _U.update(model,{accelerations: A2($List.take,30,A2($List._op["::"],_p1,model.accelerations)),currentNote: accelerationToNote(_p1)});}
   });
   var Click = {ctor: "Click"};
   var AddAcceleration = function (a) {    return {ctor: "AddAcceleration",_0: a};};
   var NoOp = {ctor: "NoOp"};
   ...
   var view = F2(function (_p2,model) {
      ...
      var _p7 = _p3._0;
      var _p6 = _p3._1;
      var drawCircle = F2(function (a,_p4) {
         var _p5 = _p4;
         return A2($Graphics$Collage.move,
         {ctor: "_Tuple2",_0: _p5.x * $Basics.toFloat(_p7),_1: _p5.y * $Basics.toFloat(_p6)},
         A2($Graphics$Collage.filled,A4($Color.rgba,128,0,0,$Basics.toFloat(30 - a) / 30),$Graphics$Collage.circle(_p5.z / 2 * $Basics.toFloat(_p7))));
      });
      return $Graphics$Element.layers(_U.list([A3($Graphics$Collage.collage,_p7,_p6,A2($List.indexedMap,drawCircle,model.accelerations))
                                              ,A2($Graphics$Input.button,A2($Signal.message,inbox.address,Click),model.mute ? "LISTEN" : "SHUTUP")]));
   });
   var initialModel = {currentNote: 0,accelerations: _U.list([]),mute: true};
   var Model = F3(function (a,b,c) {    return {currentNote: a,mute: b,accelerations: c};});
   ...
   var actions = A2($Signal.merge,inbox.signal,A2($Signal.map,AddAcceleration,A2($Signal.sampleOn,$Time.fps(30),acceleration)));
   ...
   var mute = Elm.Native.Port.make(_elm).outboundSignal("mute",function (v) {    return v;},A2($Signal.map,function (_) {    return _.mute;},state));
   var main = A3($Signal.map2,view,$Window.dimensions,state);
   var Acceleration = F3(function (a,b,c) {    return {x: a,y: b,z: c};});
   return _elm.Accelerate.values = {_op: _op
                                   ,Acceleration: Acceleration
                                   ,Model: Model
                                   ,initialModel: initialModel
                                   ,NoOp: NoOp
                                   ,AddAcceleration: AddAcceleration
                                   ,Click: Click
                                   ,update: update
                                   ,accelerationToNote: accelerationToNote
                                   ,view: view
                                   ,inbox: inbox
                                   ,actions: actions
                                   ,state: state
